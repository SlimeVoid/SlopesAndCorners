package slimevoid.slopesncorners.items.crafting;
...
public class NBTRecipe implements IRecipe {
	...
	private boolean NBTTagsMatch(NBTTagCompound tagCompound,
		...
		while (iterator.hasNext()) {
			...
			if (!(childCompound instanceof NBTTagCompound && tagCompound2
					.hasKey(childCompound.getName()))) {
				if (childCompound instanceof NBTTagByte) {
					return (((NBTTagByte) childCompound).data == ((NBTTagByte) (tagCompound2
							.getTag(childCompound.getName()))).data);
				} else if (childCompound instanceof NBTTagByteArray) {
					return (((NBTTagByteArray) childCompound).byteArray
							.equals(((NBTTagByteArray) (tagCompound2
									.getTag(childCompound.getName()))).byteArray));
				} else if (childCompound instanceof NBTTagDouble) {
					return (((NBTTagDouble) childCompound).data == ((NBTTagDouble) (tagCompound2
							.getTag(childCompound.getName()))).data);
				} else if (childCompound instanceof NBTTagFloat) {
					return (((NBTTagFloat) childCompound).data == ((NBTTagFloat) (tagCompound2
							.getTag(childCompound.getName()))).data);
				} else if (childCompound instanceof NBTTagInt) {
					return (((NBTTagInt) childCompound).data == ((NBTTagInt) (tagCompound2
							.getTag(childCompound.getName()))).data);
				} else if (childCompound instanceof NBTTagIntArray) {
					return (((NBTTagIntArray) childCompound).intArray
							.equals(((NBTTagIntArray) (tagCompound2
									.getTag(childCompound.getName()))).intArray));
				} else if (childCompound instanceof NBTTagLong) {
					return (((NBTTagLong) childCompound).data == ((NBTTagLong) (tagCompound2
							.getTag(childCompound.getName()))).data);
				} else if (childCompound instanceof NBTTagShort) {
					return (((NBTTagShort) childCompound).data == ((NBTTagShort) (tagCompound2
							.getTag(childCompound.getName()))).data);
				} else if (childCompound instanceof NBTTagString) {
					return (((NBTTagString) childCompound).data
							.equals(((NBTTagString) (tagCompound2
									.getTag(childCompound.getName()))).data));
				} else {
					return false;
				}
			} else if (!tagCompound2.hasKey(childCompound.getName())
					|| !NBTTagsMatch((NBTTagCompound) childCompound,
							(NBTTagCompound) (tagCompound2)
