...
public class BlockSlopesNCornersRenderer extends BlockSlopesRendererBase {
	...
	@Override
	protected boolean renderCustomBlockBlockWithColorMultiplier(Block block,
			int x, int y, int z, float f, float f1, float f2, int iDir,
			int model, RenderBlocks renderer, IBlockAccess world) {
		TileEntitySlopes tileentity = (TileEntitySlopes) BlockHelper.getTileEntity(world, x, y, z, ((BlockSlopesBase) block).getTileMapData(model));
		if (tileentity != null) {
			if (tileentity.GetCorneriDir() > -1) {
				return renderCornersBlockWithColorMultiplier(block, x, y, z, f, f1,
						f2, tileentity.GetCorneriDir(), renderer, world);
			} else if (tileentity.GetIntCorneriDir() > -1) {
				return renderIntCornersBlockWithColorMultiplier(block, x, y, z, f,
						f1, f2, tileentity.GetIntCorneriDir(), renderer, world);
			} else {
				return renderSlopesBlockWithColorMultiplier(block, x, y, z, f, f1,
						f2, iDir, renderer, world);
			}
		}
		return false;
	}
	...
	@Override
	protected boolean renderCustomBlockWithAmbientOcclusion(Block block, int x,
			int y, int z, float f, float f1, float f2, int iDir,
			int model, RenderBlocks renderer, IBlockAccess world) {
		TileEntitySlopes tileentity = (TileEntitySlopes) BlockHelper.getTileEntity(world, x, y, z, ((BlockSlopesBase) block).getTileMapData(model));
		if (tileentity != null) {
			if (tileentity.GetCorneriDir() > -1) {
				return renderCornersBlockWithAmbientOcclusion(block, x, y, z, f,
						f1, f2, tileentity.GetCorneriDir(), renderer, world);
			} else if (tileentity.GetIntCorneriDir() > -1) {
				return renderIntCornersBlockWithAmbientOcclusion(block, x, y, z, f,
						f1, f2, tileentity.GetIntCorneriDir(), renderer, world);
			} else {
				return renderSlopesBlockWithAmbientOcclusion(block, x, y, z, f, f1,
						f2, iDir, renderer, world);
			}
		}
		return false;
	...
	public boolean renderSlopesBlockWithAmbientOcclusion(Block block, int i,
		...
		if (renderblocks.renderAllFaces
			...
			if (renderblocks.fancyGrass && l.getIconName().equals("grass_side") && !renderblocks.hasOverrideBlockTexture()){
				...
				renderSlopesEastFace(block, i, j, k, BlockGrass.getIconSideOverlay(), iDir, renderblocks,
						block.getMixedBrightnessForBlock(iblockaccess, i, j, k));
			}
			flag = true;
		}
		if (renderblocks.renderAllFaces
			...
			if (renderblocks.fancyGrass && i1.getIconName().equals("grass_side") && !renderblocks.hasOverrideBlockTexture()){
				...
				renderSlopesWestFace(block, i, j, k, BlockGrass.getIconSideOverlay(), iDir, renderblocks,
						block.getMixedBrightnessForBlock(iblockaccess, i, j, k));
			}
			flag = true;
		}
		if (renderblocks.renderAllFaces
			...
			if (renderblocks.fancyGrass && j1.getIconName().equals("grass_side") && !renderblocks.hasOverrideBlockTexture()){
				...
				renderSlopesNorthFace(block, i, j, k, BlockGrass.getIconSideOverlay(), iDir, renderblocks,
						block.getMixedBrightnessForBlock(iblockaccess, i, j, k));
			}
			flag = true;
		}
		if (renderblocks.renderAllFaces
			...
			if (renderblocks.fancyGrass && k1.getIconName().equals("grass_side") && !renderblocks.hasOverrideBlockTexture()){
				...
				renderSlopesSouthFace(block, i, j, k, BlockGrass.getIconSideOverlay(), iDir, renderblocks,
						block.getMixedBrightnessForBlock(iblockaccess, i, j, k));
			}
			flag = true;
		}
