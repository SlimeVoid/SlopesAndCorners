...
public class SlopesEntityDiggingFX extends EntityDiggingFX{
				...
				for (int k1 = 0; k1 < b0; ++k1) {
					for (int l1 = 0; l1 < b0; ++l1) {
						...
						double d2 = (double) z + ((double) l1 + 0.5D)
								/ (double) b0;
						SlopesEntityDiggingFX particle = new SlopesEntityDiggingFX(world,
								d0, d1, d2, d0 - (double) x - 0.5D, d1
								- (double) y - 0.5D, d2 - (double) z
								- 0.5D, baseblock, MaterialsLib.getBlockDmg(tile.getSlopeIndex()));
	...
	public static boolean doBlockHitEffects(World world, MovingObjectPosition target, EffectRenderer effectRenderer, Block block) {
		TileEntitySlopesBase tile = (TileEntitySlopesBase) world.getBlockTileEntity(target.blockX, target.blockY,target.blockZ);
		Block baseblock = MaterialsLib.getBlock(tile.getSlopeIndex());		
		
			float f = 0.1F;
			double d0 = (double) target.blockX
					+ world.rand.nextDouble()
					* (baseblock.getBlockBoundsMaxX() - baseblock.getBlockBoundsMinX() - (double) (f * 2.0F))
					+ (double) f + baseblock.getBlockBoundsMinX();
			double d1 = (double) target.blockY
					+ world.rand.nextDouble()
					* (baseblock.getBlockBoundsMaxY() - baseblock.getBlockBoundsMinY() - (double) (f * 2.0F))
					+ (double) f + baseblock.getBlockBoundsMinY();
			double d2 = (double) target.blockZ
					+ world.rand.nextDouble()
					* (baseblock.getBlockBoundsMaxZ() - baseblock.getBlockBoundsMinZ() - (double) (f * 2.0F))
					+ (double) f + baseblock.getBlockBoundsMinZ();

			
			SlopesEntityDiggingFX particle = new SlopesEntityDiggingFX(world, d0, d1, d2,
					0.0D, 0.0D, 0.0D, baseblock, MaterialsLib.getBlockDmg(tile.getSlopeIndex()));
			effectRenderer.addEffect(particle
