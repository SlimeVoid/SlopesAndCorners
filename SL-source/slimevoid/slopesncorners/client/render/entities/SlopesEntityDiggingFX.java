...
public class SlopesEntityDiggingFX extends EntityDiggingFX {
	...
	public SlopesEntityDiggingFX(World par1World, double par2, double par4, double par6, double par8, double par10, double par12, Block par14Block, int par15) {
		super(par1World, par2, par4, par6, par8, par10, par12, par14Block, par15, par1World.rand
				.nextInt(6));
	}
	...
	public static boolean doBlockDestroyEffects(World world, int x, int y, int z, int meta, EffectRenderer effectRenderer, Block block) {
		TileEntitySlopesBase tile = (TileEntitySlopesBase) world
			...
			for (int j1 = 0; j1 < b0; ++j1) {
				for (int k1 = 0; k1 < b0; ++k1) {
					for (int l1 = 0; l1 < b0; ++l1) {
						...
						SlopesEntityDiggingFX particle = new SlopesEntityDiggingFX(
								world,
								d0,
								d1,
								d2,
	...
	public static boolean doBlockHitEffects(World world, MovingObjectPosition target, EffectRenderer effectRenderer, Block block) {
		TileEntitySlopesBase tile = (TileEntitySlopesBase) world
				.getBlockTileEntity(target.blockX, target.blockY, target.blockZ);
		Block baseblock = MaterialsLib.getBlock(tile.getSlopeIndex());
		if (baseblock != null) {
			float f = 0.1F;
			double d0 = (double) target.blockX + world.rand.nextDouble() * (baseblock
					.getBlockBoundsMaxX() - baseblock.getBlockBoundsMinX() - (double) (f * 2.0F)) + (double) f + baseblock
					.getBlockBoundsMinX();
			double d1 = (double) target.blockY + world.rand.nextDouble() * (baseblock
					.getBlockBoundsMaxY() - baseblock.getBlockBoundsMinY() - (double) (f * 2.0F)) + (double) f + baseblock
					.getBlockBoundsMinY();
			double d2 = (double) target.blockZ + world.rand.nextDouble() * (baseblock
					.getBlockBoundsMaxZ() - baseblock.getBlockBoundsMinZ() - (double) (f * 2.0F)) + (double) f + baseblock
					.getBlockBoundsMinZ();
			...
			SlopesEntityDiggingFX particle = new SlopesEntityDiggingFX(
					...
					MaterialsLib.getBlockDmg(tile.getSlopeIndex()));
			effectRenderer.addEffect(particle
					.applyColourMultiplier(	target.blockX,
											target.blockY,
											target.blockZ)
