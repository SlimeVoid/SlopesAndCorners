...
public class Slopes {
	...
	public class MaterialHandler implements IMaterialHandler {
		...
		public void addMaterialReference(int i) {
			...
			LanguageRegistry.instance().addStringLocalization(	(new StringBuilder()).append(name).append(".slope").append(".name").toString(),
																(new StringBuilder()).append(desc).append(" Slope").toString());

			ItemStack baseItem = MaterialsLib.getItemStack(i);
			if (baseItem != null) {
				ItemStack slopeStack = new ItemStack(ConfigurationLib.blockSlopes, 4, (BlockLib.BLOCK_SLOPES_ID << 12)
																						+ i);
				GameRegistry.addShapedRecipe(	slopeStack.copy(),
												new Object[] {
														"  B",
														" BB",
														"B B",
