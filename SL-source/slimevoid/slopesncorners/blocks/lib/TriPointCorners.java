...
public class TriPointCorners {
	...
	public class MaterialHandler implements IMaterialHandler {
		...
		public void addMaterialReference(int i) {
			...
			LanguageRegistry.instance().addStringLocalization(	(new StringBuilder()).append(name).append(".slant").append(".name").toString(),
																(new StringBuilder()).append(desc).append(" Slanted Corner").toString());
			ItemStack baseItem = MaterialsLib.getItemStack(i);
			if (baseItem != null) {
				ItemStack stack = new ItemStack(ConfigurationLib.blockSlopes, 6, (BlockLib.BLOCK_TRIPOINT_ID << 12)
																					+ i);
	...
	public class PlacementHandler implements IPlacementHandler {
		...
		public boolean onPlaceBlock(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int i) {
			...
			TileEntityTriPointCorner tileentity = (TileEntityTriPointCorner) BlockHelper.getTileEntity(	world,
																										x,
																										y,
																										z,
