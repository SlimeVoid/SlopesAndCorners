...
public class SideSlopes {
	...
	public class MaterialHandler implements IMaterialHandler {
		...
		public void addMaterialReference(int i) {
			...
			LanguageRegistry.instance().addStringLocalization(	(new StringBuilder()).append(name).append(".side").append(".name").toString(),
																(new StringBuilder()).append(desc).append(" Side Slope").toString());
			...
			if (baseItem != null) {
				...
				ItemStack stack = new ItemStack(ConfigurationLib.blockSlopes, 1, (BlockLib.BLOCK_SIDES_ID << 12)
																					+ i);
	...
	public class PlacementHandler implements IPlacementHandler {
		...
		public boolean onPlaceBlock(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int i) {
			...
			TileEntitySideSlopes tileentity = (TileEntitySideSlopes) BlockHelper.getTileEntity(	world,
																								x,
																								y,
																								z,
