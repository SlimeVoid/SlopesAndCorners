...
public class BlockOblicSlopes extends BlockVannilaBased {
	...
	public void setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess,
		...
		if(raytracing){
			this.setOblicBounds(par1IBlockAccess.getBlockMetadata(par2, par3, par4) % 8, raytraceFirstPhase);
	...
	private void setOblicBounds(int iDir,boolean firstPhase){
		if (iDir == 0) {
			if(firstPhase) setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 0.5F, 1.0F);
			else
			...
			if(firstPhase) setBlockBounds(0.0F, 0.5F, 0.0F, 1.0F, 1.0F, 1.0F);
			else
